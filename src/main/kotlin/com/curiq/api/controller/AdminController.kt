...
class AdminController(
    ...
    fun getMetrics(): Map<String, Any> {
        ...
        val queued = allJobs.count { it.status == JobStatus.QUEUED }
        val processing = allJobs.count { it.status == JobStatus.PROCESSING }
        val failed = allJobs.count { it.status == JobStatus.FAILED }
        ...
        val completedJobs = allJobs.filter { it.status == JobStatus.COMPLETED && it.processingTimeMs != null }
