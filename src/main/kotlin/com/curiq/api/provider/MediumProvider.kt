...
class MediumProvider(
    ...
    private val logger = LoggerFactory.getLogger(MediumProvider::class.java)
    ...
    override fun extractMetadata(item: SavedItem): ProviderResult? {
        ...
        logger.info("MediumProvider executing internal strategy for: ${item.url}")

        // 1. Try OpenGraph first
        var result = openGraphProvider.extractMetadata(item)
        if (isSufficient(result)) {
            logger.info("MediumProvider succeeded using OpenGraph")
            return result?.copy(
                providerName = "MediumProvider", 
        ...
        if (isSufficient(result)) {
            logger.info("MediumProvider succeeded using HTML")
            return result?.copy(
                providerName = "MediumProvider", 
        ...
        logger.info("MediumProvider falling back to Microlink")
