...
class FacebookProvider(
    ...
    private val logger = LoggerFactory.getLogger(FacebookProvider::class.java)
    ...
    override fun extractMetadata(item: SavedItem): ProviderResult? {
        ...
        logger.info("FacebookProvider executing internal strategy for: ${item.url}")

        // 1. Try OpenGraph first
        var result = openGraphProvider.extractMetadata(item)
        if (isSufficient(result)) {
            logger.info("FacebookProvider succeeded using OpenGraph")
            return result?.copy(
        ...
        if (isSufficient(result)) {
            logger.info("FacebookProvider succeeded using HTML")
            return result?.copy(
                providerName = "FacebookProvider", 
        ...
        logger.info("FacebookProvider falling back to Microlink")
