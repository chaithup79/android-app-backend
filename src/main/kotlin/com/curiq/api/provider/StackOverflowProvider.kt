...
class StackOverflowProvider(
    ...
    private val logger = LoggerFactory.getLogger(StackOverflowProvider::class.java)
    ...
    override fun extractMetadata(item: SavedItem): ProviderResult? {
        ...
        logger.info("StackOverflowProvider executing internal strategy for: ${item.url}")

        // 1. Try OpenGraph first
        var result = openGraphProvider.extractMetadata(item)
        if (isSufficient(result)) {
            logger.info("StackOverflowProvider succeeded using OpenGraph")
            return result?.copy(
                providerName = "StackOverflowProvider", 
        ...
        if (isSufficient(result)) {
            logger.info("StackOverflowProvider succeeded using HTML")
            return result?.copy(
                providerName = "StackOverflowProvider", 
        ...
        logger.info("StackOverflowProvider falling back to Microlink")
